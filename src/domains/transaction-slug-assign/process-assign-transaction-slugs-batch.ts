...
export const processAssignTransactionSlugsBatch = async (
  ...
  console.log('🚀 processAssignTransactionSlugsBatch', {
    total: totalMatching,
  ...
  if (batch.length === 0) {
    console.log('✅ processAssignTransactionSlugsBatch', 'nothing to process');
    return {
      processed: 0,
      succeeded: 0,
      failed: 0,
  ...
  for (let i = 0; i < batch.length; i += 1) {
    ...
    try {
      ...
      console.error('❌ processAssignTransactionSlugsBatch', {
        progress: `${i + 1}/${batch.length}`,
        transaction_id: txn.id,
        message,
      });
